...
def train_mappo(env: NetworkSimulator, save_dir: str, logger: Logger):
    ...
    # 创建结果可视化器
    plotter = ResultPlotter(save_dir)
    
    # 记录训练指标
    training_rewards = []
    training_success_rates = []
    training_delays = []
    training_packet_losses = []
    ...
    for episode in episode_pbar:
        ...
        # 记录训练指标
        training_rewards.append(episode_reward/max_steps)
        training_success_rates.append(delivery_rate)
        training_delays.append(avg_delay)
        training_packet_losses.append(drop_rate)
        training_throughputs.append(np.mean(metrics['throughput']) if metrics.get('throughput') else 0)
        
        # 每100个episode绘制一次训练曲线
        if (episode + 1) % 100 == 0:
            plotter.plot_training_curves(
                rewards=training_rewards,
                success_rates=training_success_rates,
                delays=training_delays,
                packet_losses=training_packet_losses,
                throughputs=training_throughputs,
...
def evaluate_mappo(env: NetworkSimulator, model_path: str, logger: Logger, num_episodes: int = 100) -> Dict[str, float]:
    """
    评估MAPPO算法
    ...
    logger.info(f"开始评估MAPPO模型: {model_path}")
    ...
    # 创建结果可视化器
    plotter = ResultPlotter(os.path.dirname(model_path))
    
    # 创建episode进度条
    episode_pbar = tqdm(range(num_episodes), desc="评估进度", unit="episode")
    ...
    for episode in episode_pbar:
        ...
        # 创建step进度条
        step_pbar = tqdm(range(env.config['simulation']['common']['max_steps']), 
                        desc=f"Episode {episode+1}", leave=False, unit="step")
        ...
        for step in step_pbar:
            ...
            # 更新统计信息
            if 'metrics' in info:
                metrics_info = info['metrics']
                
                # 更新生成的数据包数量
                if 'packets_generated' in metrics_info:
                    new_packets = max(0, metrics_info['packets_generated'] - episode_packets['generated'])
                    if new_packets > 0:
                        episode_packets['generated'] += new_packets
                        episode_packets['in_transit'] += new_packets
                
                # 更新成功传输的数据包数量
                if 'packets_delivered' in metrics_info:
                    new_delivered = max(0, metrics_info['packets_delivered'] - episode_packets['delivered'])
                    if new_delivered > 0:
                        episode_packets['delivered'] += new_delivered
                        episode_packets['in_transit'] = max(0, episode_packets['in_transit'] - new_delivered)
                        
                        # 记录延迟
                        if 'delay' in metrics_info and metrics_info['delay'] > 0:
                            metrics['delay'].append(metrics_info['delay'])
                
                # 更新丢弃的数据包数量
                if 'packets_dropped' in metrics_info:
                    new_dropped = max(0, metrics_info['packets_dropped'] - episode_packets['dropped'])
                    if new_dropped > 0:
                        episode_packets['dropped'] += new_dropped
                        episode_packets['in_transit'] = max(0, episode_packets['in_transit'] - new_dropped)
                        ...
                        if action == -1:
                            drop_reasons['no_valid_path'] += new_dropped
                        if metrics_info.get('buffer_overflow', False):
                            drop_reasons['buffer_overflow'] += new_dropped
                        if metrics_info.get('link_failure', False):
                            drop_reasons['link_failure'] += new_dropped
                        if not (action == -1 or metrics_info.get('buffer_overflow', False) or metrics_info.get('link_failure', False)):
                            drop_reasons['other'] += new_dropped
                
                # 更新其他指标
                if 'throughput' in metrics_info:
                    metrics['throughput'].append(metrics_info['throughput'])
                if 'link_utilization' in metrics_info:
                    metrics['link_utilization'].append(metrics_info['link_utilization'])
        ...
        episode_pbar.set_postfix({
            ...
            '成功率': f"{delivery_rate:.1f}%",
            '丢包率': f"{drop_rate:.1f}%",
            '延迟': f"{np.mean(metrics['delay']):.2f}s" if metrics['delay'] else "N/A"
        })
        ...
        if (episode + 1) % 10 == 0:
            ...
            if metrics['delay']:
                logger.info(f"\n延迟统计:")
                logger.info(f"  - 平均延迟: {np.mean(metrics['delay']):.3f}秒")
                logger.info(f"  - 最小延迟: {np.min(metrics['delay']):.3f}秒")
                logger.info(f"  - 最大延迟: {np.max(metrics['delay']):.3f}秒")
                logger.info(f"  - 延迟标准差: {np.std(metrics['delay']):.3f}秒")
            
            logger.info(f"\n丢包原因分析:")
            for reason, count in drop_reasons.items():
                percentage = (count / max(1, dropped_packets)) * 100
                logger.info(f"  - {reason}: {count}个 ({percentage:.2f}%)")
    ...
    results = {
        ...
        'mean_delay': np.mean(metrics['delay']) if metrics['delay'] else 0,
        'mean_throughput': np.mean(metrics['throughput']) if metrics['throughput'] else 0,
        'mean_packet_loss': drop_rate,
        'mean_link_utilization': np.mean(metrics['link_utilization']) if metrics['link_utilization'] else 0
    }
    
    # 绘制评估结果
    plotter.plot_evaluation_results(results, 'MAPPO')
    
    # 保存评估结果
    plotter.save_results(results, 'MAPPO')
...
def run_ospf(env: NetworkSimulator, save_dir: str, logger: Logger):
    """
    运行OSPF算法
    ...
    algorithm = OSPFAlgorithm(ospf_config)
    env.set_routing_algorithm(algorithm)
    ...
    for episode in range(max_episodes):
        ...
        # 每个episode的统计
        episode_packets = {
            'generated': 0,  # 生成的数据包总数
            'delivered': 0,  # 成功传输的数据包数
            'dropped': 0,    # 丢弃的数据包数
            'in_transit': 0  # 传输中的数据包数
        }
        ...
        for step in range(max_steps):
            ...
            if 'metrics' in info:
                ...
                # 更新生成的数据包数量
                if 'packets_generated' in metrics:
                    new_packets = metrics['packets_generated'] - episode_packets['generated']
                    if new_packets > 0:
                        episode_packets['generated'] += new_packets
                        episode_packets['in_transit'] += new_packets
                ...
                # 更新成功传输的数据包数量
                if 'packets_delivered' in metrics:
                    new_delivered = metrics['packets_delivered'] - episode_packets['delivered']
                    if new_delivered > 0:
                        episode_packets['delivered'] += new_delivered
                        episode_packets['in_transit'] = max(0, episode_packets['in_transit'] - new_delivered)
                ...
                # 更新丢弃的数据包数量
                if 'packets_dropped' in metrics:
                    new_dropped = metrics['packets_dropped'] - episode_packets['dropped']
                    if new_dropped > 0:
                        episode_packets['dropped'] += new_dropped
                        episode_packets['in_transit'] = max(0, episode_packets['in_transit'] - new_dropped)
        ...
        # 打印丢包原因分析
        logger.info("\n丢包原因分析:")
        total_drops = episode_packets['dropped']
        if total_drops > 0:
            for reason, count in drop_reasons.items():
                percentage = (count / total_drops * 100)
                logger.info(f"{reason}: {count}个 ({percentage:.2f}%)")
        ...
        training_success_rates.append(success_rate)
        training_delays.append(avg_delay)
        training_packet_losses.append(episode_packets['dropped'] / max(1, episode_packets['generated']))
        training_throughputs.append(metrics.get('throughput', 0))
    ...
    results = {
        ...
        'mean_delay': np.mean(training_delays),
        'mean_throughput': np.mean(training_throughputs),
        'mean_packet_loss': np.mean(training_packet_losses),
        'mean_link_utilization': np.mean(training_success_rates) / 100.0
    }
    
    plotter.plot_evaluation_results(results, 'OSPF')
    plotter.save_results(results, 'OSPF')
