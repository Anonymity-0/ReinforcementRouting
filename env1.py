...
class SatelliteEnv:
    def __init__(self, service_type='delay_sensitive', multi_agent=True):
        ...
        # 划分智能体
        self.agents = self._partition_agents()
    ...
    def _partition_agents(self):
        agents = {}
        if self.multi_agent:
            for sat_idx in range(self.n_leo):
                agents[sat_idx] = [sat_idx]
        else:
            agents[0] = list(range(self.n_leo))
        return agents
    ...
    def get_observation(self):
        if self.multi_agent:
            # 返回每个智能体的缓存状态
            observations = {}
            for agent_id, sats in self.agents.items():
                obs = np.mean(self.cache_state[sats])  # 示例：每个智能体获取其控制卫星的平均缓存状态
                observations[agent_id] = [obs]
            return observations
        else:
            # 返回单智能体的观察
            return {'agent_0': self.cache_state.tolist()}
    ...
    def step(self, actions):
        if self.multi_agent:
            ...
            if not candidate_paths:
                ...
                reward = -1  # 或者给予一个适当的奖励/惩罚
                return next_state, reward, done
            action = actions.get('agent_0', 0)  # 获取单智能体的动作
            if action >= len(candidate_paths):
                action = 0  # 防止越界
            path = candidate_paths[action]
            # 计算 QoS 指标
